      ...
      <div @click="redirect(card.route)" class="card padding-10">
        ...
        <div class="card-product-image-container">
          ...
          <div
            ...
            @click="openQuickView({details: card, event: $event})"
          >
        ...
        <div class="card-bottom-container">
          <div class="card-body no-padding">
            ...
            <div class="fs18 card-price-container">
              <span
                class="card-current-price fw6 mr10"
              >{{ card['currency-icon'] }} {{ card.max_price }}</span>

              <template v-if="card.price > card.max_price">
                <span class="card-actual-price mr10">{{ card['currency-icon'] }} {{ card.price }}</span>

                <span
                  class="card-discount"
                >{{ ((card.price - card.max_price) * 100) / card.price }}%</span>
              </template>
            </div>
          </div>
